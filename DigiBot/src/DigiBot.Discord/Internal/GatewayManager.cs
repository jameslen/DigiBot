...
namespace DigiBot.Discord
{
    ...
    public class GatewayManager
    {
        ...
        public GatewayManager(string gateway, string token, CancellationToken cancel, ILogger logger = null)
        {
            m_token = token;
            m_gateway = gateway;
        ...
        public void Initialize(int currentShard, int totalShards)
        {
            _currentShard = currentShard;
            _totalShards = totalShards;

            Task.Run(async () =>
            {
                await SocketLoopHandler(currentShard, totalShards);
        ...
        private async Task SocketLoopHandler(int currentShard, int totalShards)
        {
            ...
            while (m_gatewaySocket.State == WebSocketState.Open)
            {
                try 
                {
                    ...
                    var recv = await m_gatewaySocket.ReceiveAsync(buffer, _cancelToken);
        ...
        private bool AttemptConnection(string gateway)
        {
            ...
            for(int i = 0; i < 3; ++i)
            {
                ...
                m_gatewaySocket.ConnectAsync(new Uri(gateway + "?v=5&encoding=json"), _cancelToken).Wait();
        ...
        {
            var response = CreateIdentity(m_token, _currentShard, _totalShards);
            SendData(response);
            SendHeartbreat();
        }
        ...
        private void SendData(string payload)
        {
            ...
            m_gatewaySocket.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(payload)), WebSocketMessageType.Text, true, _cancelToken);
        }
